...
class NetworkTestBehavior(unittest.TestCase):
    ...
    def test_std_project(self):
        ...
        def build_network():
            ...
            input_layer  = leabra.Layer(25, spec=layer_spec, unit_spec=u_spec, name='input_layer')
            hidden_layer = leabra.Layer(25, spec=layer_spec, unit_spec=u_spec, name='hidden_layer')
            output_layer = leabra.Layer(25, spec=layer_spec, unit_spec=u_spec, name='output_layer')
        ...
        for name in logs.keys():
            for t, (py, em) in enumerate(list(zip(logs[name], table_data[name]))[:10]):
